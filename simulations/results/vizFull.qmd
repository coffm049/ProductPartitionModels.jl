...
df <- data.frame("file" = dir()) %>%
    ...
    # unnest them
    unnest(data) %>%
    filter(common == 1.0, interEffect == 0.5, xdiff == 1.0) %>%
    select(
...
temp <- df %>%
    reframe(
        ...
        .by = c(Common, InterEffect, Alpha, Beta, Precision, Imbalanced)
    ) %>%
    pivot_longer(
        -c(Common, Alpha, Beta, Precision, InterEffect, Imbalanced),
...
longDF %>%
    filter(Metric == "common", Imbalanced == FALSE, Alpha >=2.0) %>%
    ggplot(aes(x = Statistic, fill = Model)) +
...
temp %>%
    ...
    tab_spanner(
        label = "Simulation",
        columns = c(Common, InterEffect, Imbalanced)
    ) |>
    tab_spanner(
        label = "Priors",
...
longDF %>%
    ...
    ggplot(aes(x = Statistic, fill = Model)) +
    geom_density(alpha = 0.3) +
    facet_grid(rows = vars(InterEffect), cols = vars(Common), scales = "free")
```
