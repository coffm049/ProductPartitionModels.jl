...
temp <- df %>%
    ...
    reframe(
        ...
        .by = c(N, Common, InterEffect, Alpha, Beta, Precision, Imbalanced, variance)
    ) %>%
    pivot_longer(
...
longDF <- df %>%
    pivot_longer(
        -c(N, Common, Alpha, Beta, Precision, InterEffect, Imbalanced, variance),
         names_to = c(".value", "Metric"),
         names_pattern = "(.*)\\_(.*)",
        ) %>%
...
longDF %>%
    filter(Imbalanced == FALSE, Metric == "common1", InterEffect == 0.25) %>%
    ggplot(aes(x = Statistic, fill = Model)) +
    ...
    xlab("Common Estimate")
ggsave("media/beta1Tuning.png", width = 6, dpi = 300, units = "in")

longDF %>%
    filter(Imbalanced == FALSE, Metric == "common2", InterEffect == 0.25) %>%
    ggplot(aes(x = Statistic, fill = Model)) +
    geom_density(alpha = 0.2) +
    geom_vline(xintercept = -1.0) +
    geom_vline(xintercept = 0.0) +
    ...
    ggtitle("Common effects (common=-1, Interaction=0.25), Balanced") +
    xlab("Common Estimate")
ggsave("media/beta2Tuning.png", width = 6, dpi = 300, units = "in")
```

```{r}
longDF %>%
    filter(Imbalanced == FALSE, grepl("^common", Metric), Alpha == 0.1, Beta==0.1) %>%
    ggplot(aes(x = Statistic, fill = Model)) +
    geom_density(alpha = 0.2) +
    geom_vline(xintercept = 0.0) +
    facet_grid(
        rows = vars(InterEffect), cols = vars(Metric), scales = "free",
        labeller = labeller(.rows = label_both, .cols = label_both)
    ) +
    ggtitle("Common effects (common=-1, Interaction=0.25), Balanced") +
    xlab("Common Estimate")
ggsave("media/betaVinter.png", width = 6, dpi = 300, units = "in")
```
...
```{r}
longDF %>%
    filter(Alpha == 0.1, Beta == 0.1, grepl("rind", Metric) | grepl("bayes", Metric)) %>%
    select(-c(Alpha, Beta, Precision, variance, Common, Imbalanced)) %>%
