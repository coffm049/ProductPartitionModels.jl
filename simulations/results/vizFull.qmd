...
temp <- df %>%
    ...
    select(-c(`ppmx-common`, kmean, slr, ppmx))
    mutate(
        `ppmx-common1` = coalesce(`ppmx-common1`, `ppmx-common`),
        `ppmx-common2` = coalesce(`ppmx-common2`, `ppmx-common`),
        kmean1 = coalesce(`ppmx-common1`, `ppmx-common`)
    ) %>%
    select(-c(`ppmx-common`, kmean))
longDF <- df %>%
    pivot_longer(
...
longDF %>%
    ...
    facet_grid(
        rows = vars(xdiff, Alpha), cols = vars(Beta), scales = "free",
        labeller = labeller(.rows = label_both, .cols = label_both)
    ) +
...
temp %>%
    ...
    arrange(xdiff, Alpha, Beta, Metric) |>
    group_by(xdiff, Alpha,Beta) |>
