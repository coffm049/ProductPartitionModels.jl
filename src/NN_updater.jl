...
# Random.seed!(123)
# N, p = 7, 3
# true_mu = [0.25, 0.25, 0.25]
# true_sigma2 = [0.1, 0.1, 0.1]
# # X = hcat([rand(Normal(true_mu[j], sqrt(true_sigma2[j])), N) for j in 1:p]...)
# X = repeat([quantile(Normal(0.25, 0.1), i/(N + 1)) for i in 1:N], inner = (1,3))
# 
# # Prior hyperparameters
# mu0 = [0.0, 0.0, 0.0]  # Prior mean
# kappa0 = [1e-2, 1e-2, 1e-2]  # Prior precision
# alpha0 = [30.0, 30.0, 30.0]  # Prior shape for σ^2
# beta0 = [1e-4, 1e-4, 1e-4]  # Prior scale for σ^2
# 
# # Number of posterior samples
# nsamples = 100
# 
